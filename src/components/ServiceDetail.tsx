...
function OfferingGallery({
	...
	// Auto-play suave cuando no hay interacción y hay más de 1 imagen
	useEffect(() => {
	...
	return (
		...
		>
			...
			{/* Controles de flechas anterior y siguiente */}
			<button
				type="button"
				onClick={prevImage}
				aria-label="Imagen anterior"
				className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/45 hover:bg-black/75 text-white backdrop-blur-sm flex items-center justify-center transition-all duration-200 opacity-90 group-hover:opacity-100 hover:scale-110 z-10 cursor-pointer shadow-md"
			>
				<ChevronLeft size={20} />
			</button>
			<button
				...
				className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/45 hover:bg-black/75 text-white backdrop-blur-sm flex items-center justify-center transition-all duration-200 opacity-90 group-hover:opacity-100 hover:scale-110 z-10 cursor-pointer shadow-md"
			>
				<ChevronRight size={20} />
			</button>

			{/* Badge contador elegante */}
			<div className="absolute top-3 right-3 z-10 bg-black/50 backdrop-blur-sm text-white text-xs font-semibold px-2.5 py-1 rounded-full shadow-sm">
				{currentIndex + 1} / {images.length}
			</div>

			{/* Puntos de navegación (Dots) */}
			<div className="absolute bottom-3.5 left-1/2 -translate-x-1/2 z-10 flex items-center gap-1.5 bg-black/40 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-sm">
				{images.map((_, i) => (
					<button
						key={i}
						type="button"
						...
						className={`h-2 rounded-full transition-all duration-300 cursor-pointer ${
							currentIndex === i ? "w-6 bg-primary" : "w-2 bg-white/70 hover:bg-white"
						}`}
					/>
				))}
			</div>
		...
		<>
			...
			<section id="que-ofrecemos" className="bg-white">
				{offerings.map((offering, index) => {
					...
					// If service defines specific offeringImages for this index, use them; otherwise fallback
					const offeringImages =
						service.offeringImages && service.offeringImages[index] && service.offeringImages[index].length > 0
							? service.offeringImages[index]
							: [service.heroImages[(index % (service.heroImages.length - 1)) + 1] || service.heroImages[0]];
						...
						>
							<div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
								<div className="grid md:grid-cols-2 gap-8 lg:gap-14 items-center">
									{/* Contenedor de Imagen con Soporte Multimágen / Galería */}
									<motion.div
									...
									>
										<OfferingGallery images={offeringImages} title={offering.title} />
									</motion.div>
